    ...
    <div class="pt-20 p-4 sm:p-6">
        <div class="bg-white p-4 sm:p-6 rounded-lg shadow-lg">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                        ...
                        <div>
                            ...
                            <select name="duration" id="duration"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                <option value="7" {{ ($forecastDurationInput ?? 7) == 7 ? 'selected' : '' }}>7 Hari
                                </option>
                                <option value="30" {{ ($forecastDurationInput ?? 7) == 30 ? 'selected' : '' }}>30 Hari
                                </option>
                                <option value="90" {{ ($forecastDurationInput ?? 7) == 90 ? 'selected' : '' }}>3 Bulan
                                </option>
                                <option value="180" {{ ($forecastDurationInput ?? 7) == 180 ? 'selected' : '' }}>6 Bulan
                                </option>
                                <option value="365" {{ ($forecastDurationInput ?? 7) == 365 ? 'selected' : '' }}>1 Tahun
                                </option>
                            </select>
                        </div>
                        ...
                        <div class="flex items-center space-x-2">
                            <div class="flex-grow hidden">
                                <label for="frequency"
                                    class="block mb-1 text-xs font-medium text-gray-700">Frekuensi</label>
                                <select name="frequency" id="frequency"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                    <option selected value="D" {{ ($timeFrequency ?? 'D') == 'D' ? 'selected' : '' }}>
                                        Harian</option>
                                </select>
                            </div>
                            <button type="submit"
                                class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 h-fit self-end">
                                Buat Prediksi
                            </button>
                        </div>
                    </div>
            ...
            <div class="mt-4">
                ...
                @else
                    <div class="mb-4">
                        <h3 class="text-xl font-semibold text-gray-800">Hasil Prediksi untuk:
                        ...
                        <div class="text-sm text-gray-600" id="forecastSummaryText">
                            @if ($forecastData && isset($forecastData['data']))
                                @php
                                    ...
                                    $averageForecasted =
                                        count($forecastData['data']) > 0
                                            ? $totalForecasted / count($forecastData['data'])
                                            : 0;
                                    $freqText =
                                        strtolower($timeFrequency ?? 'm') == 'd'
                                            ...
                                            : (strtolower($timeFrequency ?? 'm') == 'w'
                                                ...
                                                : 'bulanan');
                                @endphp
                                Prediksi total pengurangan <strong>{{ $totalForecasted }} unit</strong> dalam
                                {{ count($forecastData['data']) }} periode {{ $freqText }} kedepan.
                            @endif
                        </div>
                    </div>
                    ...
                    options: {
                        ...
                        scales: {
                            ...
                            x: {
                                title: {
                                    display: true,
                                    text: `Periode Waktu (${currentFrequency})`
                                },
                                ticks: {
                                    maxRotation: 45,
                                    minRotation: 45,
